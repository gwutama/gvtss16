var app = (function() {
    ...
    // The orbit angle of the light sources/spotlights.
    var spotlightsOrbitTheta = 0;
    ...
    function initEventHandler() {
        ...
        window.onkeydown = function(evt) {
            ...
            case('R'):
                ...
                camera.zAngle = 0;
                camera.distance = 4;
                illumination.light[0].position = [3, 1., 0];
                illumination.light[1].position = [-3, 1., 0];                    
                break;                
            }
    ...
    /**
     * Orbit the two light sources/spotlights in the scene.
     */
    function calculateSpotlightOrbit() {
        if (spotlightsOrbitTheta >= 2*Math.PI) {
            spotlightsOrbitTheta = 0;
        }
        ...
        spotlightsOrbitTheta += step;

        var x = h + r * Math.cos(spotlightsOrbitTheta);
        var y = k - r * Math.sin(spotlightsOrbitTheta);        
        illumination.light[0].position = [x, 1., y];
        illumination.light[1].position = [-x, 1., -y];                    
	}
